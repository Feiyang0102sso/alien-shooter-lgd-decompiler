...
目前我们手上有两个版本的编译器 （sigma的保密工作做的十分甚至九分地好，时不时送你一点内部工具。当然我怀疑也可能和合作商`8-floor-games`有关，和他们合作的版本泄露过不少好东西，所以在大概22-23年之后sigma就解除了和他们的何做关系），方便起见就叫他们新版和旧版。本文主要分析都基于新版制作
...
| `+4`                | 4 bytes  | **Extern ID / Offset / Padding** | **核心多路复用字段（内置系统 ID / 相对代码段偏移）**。<br />● **Extern 内置函数 (`Type=02`)**：记录其对应的引擎底层系统函数 ID。<br />● **普通自定义函数 (`Type=03`)**：记录该函数在第四部分（Bytecode 代码段）内部对应的**字节码相对偏移量 (Relative Offset)**。<br />*(例子：文件的绝对物理偏移 = 代码段在文件中的起始物理偏移 `0xC719` + 相对字节码偏移 `0x2ED4` = 实际反编译代码中的入口物理地址 `0xF5ED`)*。<br />● **全局变量 (`Type=01`)**：无实质作用，固定填充为 `00 00 00 00`。 |
