"""
tests/test_optimizer/test_unstatic_array.py
测试非 static 局部数组的声明和逐元素赋值合并。

利用 lgc_session 夹具驱动测试。
"""
import pytest

@pytest.fixture
def generated_lgc_content(lgc_session):
    """
    对全局夹具进行局部过滤。
    因为下方的测试类写死了大量硬编码断言，只有针对特定数据集才有效。
    """
    content, asm_name = lgc_session
    if "unstatic_arr" not in asm_name:
        pytest.skip(f"当前驱动文件 [{asm_name}] 不适用于数组静态化硬编码测试，自动跳过。")
    return content
...
class TestUnstaticArrayDeclaration:
    ...
    def test_unstatic_no_init_value_int(self, generated_lgc_content):
        """Is_Initialized=False 且无赋值的 int 数组不应有 static，也无初始化。"""
    ...
    def test_unstatic_no_init_value_string(self, generated_lgc_content):
        """Is_Initialized=False 且无赋值的 string 数组不应有 static，也无初始化。"""
    ...
    def test_no_empty_unstatic_when_has_init(self, generated_lgc_content):
        ...
        for line in lines:
            if line.strip() == "int main_local16[6];":
                pytest.fail("Found un-initialized declaration for main_local16 that should have init values")
...
class TestArrayElementMerge:
    ...
    def test_line_comments_deduplicated(self, generated_lgc_content):
        """重复的 line 注释应已去重，每个 Line N 只出现一次。"""
        lines = generated_lgc_content.split("\n")
